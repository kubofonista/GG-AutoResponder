 ...
 * Specyfikacja dostępna na stronie http://boty.gg.pl/
 *
 * @version 2.2 $Id: MessageBuilder.php 2011-04-04 13:00:00Z mbaginski $
 * @copyright Copyright &copy; 2011, GG Network S.A., http://www.gadu-gadu.pl/
 * @author Marcin Bagiński <boty@gadu-gadu.pl>
...
/**
 * @brief Reprezentacja wiadomości i/lub statusu
 */
class MessageBuilder
{
		...
		if ($this->format!==NULL) {
			$this->format.=pack(
					...
					(($formatBits & FORMAT_BOLD_TEXT)) |
						...
						(($R!=$this->R || $G!=$this->G || $B!=$this->B) * 0x08)
				);
			...
			if (strlen($s)) {
				...
				foreach ($heap as $h) {
					switch ($h[0]) {
						...
						case 'color': { $flags|=0x08; $c=$h[1]; break; }
					}
				}
	...
	public function addImage($fileName, $isFile=true)
	{
		...
		if ($this->format!==NULL)
			$this->format.=pack('vCCCII', strlen($this->text), 0x80, 0x09, 0x01, strlen($this->img), $this->imgCrc);
	...
	{
		if ($this->text===NULL) $this->text=strip_tags($this->html);
